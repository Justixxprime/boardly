// Checks generate-proposal-draft, generate-cv-draft and board-assistant: no token or a bad token
...
for (const [name, body] of [["generate-proposal-draft",{brief:"A website for a club"}],["generate-cv-draft",{background:"I was a teacher for 5 years"}],["board-assistant",{message:"add a task",tasks:[]}]]) {
  ...
  let got=false; for(let i=0;i<80;i++){ if((await call()).status===429){got=true;break;} } ok(`${name}: hourly limit applies`, got);
