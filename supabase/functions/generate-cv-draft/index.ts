...
// caller is a real user here. The small hourly limit lives in this
// isolate's memory (a speed bump, not a hard guarantee).
...
async function requireUser(req: Request): Promise<{ userId: string } | Response> {
  ...
  if (aiOverLimit(data.user.id)) return reply(429, "You've used AI writing a lot in the last hour. Try again a little later.");
