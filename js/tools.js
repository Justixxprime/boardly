...
/* ---------------------------------------------------------------------
   PHOTO & VIDEO QUALITY BOOSTER
   No AI here - just the classic non-AI toolkit: a high-quality canvas
   upscale, a contrast/saturation pass, and an unsharp mask for photos;
   denoise + lanczos upscale + unsharp + eq filters (all built into
   ffmpeg) for video. Same "runs on this device" philosophy as the
   metadata remover above, and reuses its ffmpeg engine/helpers.
--------------------------------------------------------------------- */
function initQualityBooster() {
  initImageQualityTool();
  initVideoQualityTool();
}

function enhancedFileName(name) {
  const dot = name.lastIndexOf(".");
  return dot === -1 ? `${name}-enhanced` : `${name.slice(0, dot)}-enhanced${name.slice(dot)}`;
}
...
function initImageQualityTool() {
  const input = document.getElementById("quality-image-input");
  if (!input) return;
  const enhanceBtn = document.getElementById("quality-image-btn");
  const status = document.getElementById("quality-image-status");
  const previewWrap = document.getElementById("quality-image-preview");
  const previewImg = document.getElementById("quality-image-preview-img");
  const resultWrap = document.getElementById("quality-image-result");
  const beforeImg = document.getElementById("quality-image-before-img");
  const afterImg = document.getElementById("quality-image-after-img");
  let currentFile = null;

  input.addEventListener("change", () => {
    currentFile = input.files?.[0] || null;
    enhanceBtn.disabled = !currentFile;
    status.textContent = "";
  ...
  enhanceBtn.addEventListener("click", async () => {
    ...
    try {
      const blob = await enhanceImageQuality(currentFile);
      downloadBlob(blob, enhancedFileName(currentFile.name));
      beforeImg.src = previewImg.src;
      afterImg.src = URL.createObjectURL(blob);
      resultWrap.classList.remove("hidden");
      status.textContent = "Done - sharpened, color-corrected, and download started.";
      toast("Photo enhanced", "ok");
    } catch (err) {
      status.textContent = "Couldn't process that image: " + (err.message || "unknown error");
      toast("Couldn't process that image", "error");
    }
...
// The pipeline, in order: (1) upscale up to 2x with the browser's
// high-quality resampler, capped so a huge source photo can't blow up
// into a canvas too big for the browser to hold; (2) a contrast/
// saturation/brightness lift via a canvas filter; (3) an unsharp mask
// - blur a copy, then push every pixel away from that blur - which is
// the same classic sharpening trick darkroom and Photoshop techniques
// have used for decades, no AI involved.
function enhanceImageQuality(file) {
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      try {
        ...
        const scale = longSide * 2 > MAX_DIMENSION ? Math.max(1, MAX_DIMENSION / longSide) : 2;
        ...
        upCtx.drawImage(img, 0, 0, outW, outH);

        const colorCanvas = document.createElement("canvas");
        colorCanvas.width = outW; colorCanvas.height = outH;
        const colorCtx = colorCanvas.getContext("2d");
        colorCtx.filter = "contrast(108%) saturate(114%) brightness(101%)";
        colorCtx.drawImage(upCanvas, 0, 0);
        ...
        const sharp = colorCtx.getImageData(0, 0, outW, outH);
        const blurred = blurCtx.getImageData(0, 0, outW, outH);
        const sData = sharp.data, bData = blurred.data;
        for (let i = 0; i < sData.length; i += 4) {
          sData[i] = clampByte(sData[i] + SHARPEN_AMOUNT * (sData[i] - bData[i]));
          sData[i + 1] = clampByte(sData[i + 1] + SHARPEN_AMOUNT * (sData[i + 1] - bData[i + 1]));
          sData[i + 2] = clampByte(sData[i + 2] + SHARPEN_AMOUNT * (sData[i + 2] - bData[i + 2]));
          // alpha (i + 3) is left alone
        }
        colorCtx.putImageData(sharp, 0, 0);
...
function initVideoQualityTool() {
  const input = document.getElementById("quality-video-input");
  if (!input) return;
  const enhanceBtn = document.getElementById("quality-video-btn");
  const status = document.getElementById("quality-video-status");
  const progressWrap = document.getElementById("quality-video-progress-wrap");
  const progressBar = document.getElementById("quality-video-progress-bar");
  let currentFile = null;
  ...
  enhanceBtn.addEventListener("click", async () => {
    ...
    status.textContent = "Loading the video engine (first time only, about 30MB)…";
    try {
      const blob = await enhanceVideoQuality(currentFile, (progress) => {
        const pct = Math.round(progress * 100);
        progressBar.style.width = `${pct}%`;
        status.textContent = `Enhancing… ${pct}%`;
      });
      downloadBlob(blob, enhancedFileName(currentFile.name).replace(/\.[^.]+$/, ".mp4"));
      status.textContent = "Done - denoised, sharpened, upscaled, and download started.";
      toast("Video enhanced", "ok");
    } catch (err) {
      status.textContent = "Couldn't process that video: " + (err.message || "unknown error");
      toast("Couldn't process that video", "error");
    }
...
// Filter chain, left to right: hqdn3d denoises first so the sharpen
// step afterward doesn't just sharpen noise; scale upscales 1.5x with
// the lanczos resampler (a proper, non-AI upscaling algorithm, not a
// cheap nearest-neighbor stretch); unsharp is the same "blur, then
// push away from the blur" sharpening trick as the photo tool; eq
// gives the final contrast/saturation lift. Always re-encoded with
// libx264 and always saved as MP4, since the filters require a real
// re-encode regardless of the source container.
function enhanceVideoQuality(file, onProgress) {
  const FILTER_CHAIN = "hqdn3d=1.5:1.5:6:6,scale=iw*1.5:ih*1.5:flags=lanczos,unsharp=5:5:0.8:5:5:0.0,eq=contrast=1.08:saturation=1.12";
  return (async () => {
    const { fetchFile } = await import("https://esm.sh/@ffmpeg/util@0.12.2");
    const ffmpeg = await loadFFmpegEngine();
    const ext = (file.name.match(/\.[^.]+$/)?.[0] || ".mp4").toLowerCase();
    const inputName = "quality-input" + ext;
    const outputName = "quality-output.mp4";
