...
| F21 | Low | `get-shared-board` uses `select("*")` for the normal public share view (not the client portal), so anyone with a share link also receives every task column, including attachment links and internal fields. Found during F5. | Open. After step B the file links in that response stop working, but the function should still return an explicit column list. |
