...
| F17 | Medium | `marketplace-release-payment` checks the booking is `paid_held`, sends the Paystack transfer, and only then marks it `released`. Two requests at the same moment could both pass the check and send the money twice. | Open. Fix: claim the booking first with a conditional update to a new `releasing` status (needs a schema change), then transfer. |
