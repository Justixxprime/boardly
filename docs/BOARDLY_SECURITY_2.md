...
|---|---|---|---|
| F1 | High | A provider could read a booking's `access_token` from their own bookings (schema_v33 policy plus `select *`). That token is the client's key to release held money, so a provider could release the payment to themselves. | Confirmed live before the fix (a provider could read the token). Fix written and tested in a rolled-back transaction on the live database: `schema_v77_hide_booking_token_from_provider.sql` plus an explicit column list in `js/marketplace.js`. Not applied yet, it waits for the frontend to be pushed, otherwise the provider Bookings tab goes blank. |
...
| F14 | Low | `create-invoice-payment` builds Paystack's return link from an `origin` sent by the browser. A crafted request could send a payer back to a look-alike site after paying. | Open. Fix: use a fixed site address, as the find-my-booking function does. |
| F15 | Low | Not a security issue, but found while testing: an applicant who applies to the same job a second time gets a row level security error, because only the poster has an UPDATE policy. | Open. |
