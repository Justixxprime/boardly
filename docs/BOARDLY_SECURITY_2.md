...
| F5 | Medium | The `task-attachments` storage bucket is public (the old setup guide said to turn Public on). Anyone with a file's URL can open it. Uploads are limited to a user's own folder, reads were not. | Step A live (21 Sep 2026): `schema_v90_private_attachments_prep.sql` adds `can_access_task_attachment()` (board owner and accepted members can read files of tasks on their board, an uploader can read their own folder), a read rule using it, and `proof_comments.attachment_path`. The bucket is still public until step B. Code in the v13 zip signs links (6 hours, one batch call per board load) and saves only stable paths. Step B, `schema_v91_private_attachments_flip.sql` (bucket private, public read rule removed), runs only after the frontend is pushed and a real browser test passes. Rolled-back SQL test 14 of 14, `supabase/tests/private-attachments-ui.test.mjs` 43 of 43. Not yet tried in a real browser. |
| F6 | Medium | No multi-factor sign-in in Boardly. Supabase Auth supports it, it is not wired up. | Step A live (23 Sep 2026). Enrollment and removal live in Settings > Security (`js/mfa.js`, wired in `js/settings.js`) using Supabase Auth's own TOTP factors, no new Boardly table. `login.html` asks for a code after the password step when an account has a verified factor (`js/auth.js`). The real gate is `schema_v93_mfa_enforcement.sql`: a `requires_aal2()` function plus a restrictive RLS policy (Supabase's own "opted-in" pattern) on `invoices`, `transactions`, and `marketplace_provider_payouts`, live on the database now. A user with MFA off is unaffected; a user with it on gets refused on all of select/insert/update/delete on those three tables unless the session is aal2, no matter what the browser's JavaScript does. Does not touch Edge Functions using the service role key, which bypass RLS by design. Rolled-back SQL test confirmed the policy is RESTRICTIVE, applies to ALL commands, for the authenticated role, and left the existing permissive policies on all three tables untouched (3 of 3 policies present after, same as before). Turning two-factor off requires a fresh code (re-challenges to aal2 first), not just a click. Not yet exercised with a real authenticator app in a browser. Step B (open): the "opted-in" restrictive policy above was applied to the three money tables only, not inventoried across the rest of the schema (90+ migrations). Widening it to more tables (boards, tasks, clients, etc.) is a deliberate later pass, not assumed done here. |
...
| F9 | Low | No rate limiting on public endpoints such as `submit-request`, `submit-custom-form`, `roadmap-vote`. | Fixed and live (`schema_v89_rate_limits.sql`). A `rate_limit_hit` database function counts calls per window and only the service role can call it. Six public functions now use it: `submit-request`, `submit-custom-form`, `roadmap-vote`, `respond-to-proposal`, `client-portal-action`, `marketplace-find-bookings-by-email`. People are counted by a one-way hash of their IP address, so no raw IP is stored, plus a second limit per link token (or per email). If the counter itself fails, the request is let through, so a database hiccup cannot lock real people out. Live check on 21 Sep 2026: 12 quick calls to `submit-request` gave 10 normal answers and 2 "too many requests" (429). Limits are 10 to 60 calls per 10 minutes per IP depending on the function. Not covered: the other public read functions (`get-*-info`), and someone using many different IP addresses. |
| F10 | Medium | There are about 343 `innerHTML` uses in `js/`. Some paths escape, and this pass did not audit them for XSS. | Audited and fixed on 21 Sep 2026, in code (not yet live until the frontend is pushed). Two real problems found. (1) All 17 `escapeHTML` style helpers used the "put it in a div and read innerHTML back" trick, which does NOT escape double or single quotes. Every use inside an attribute, like `title="${escapeHTML(x)}"`, could be broken out of with a quote and an `onmouseover=`. All 17 now also escape quotes. (2) Links typed by people (attachment links, published links, portfolio, meeting and staging links) were put into `href` and `src` with no check, so a `javascript:` link would run when clicked. A new `safeUrl()` in `js/supabase-client.js` only lets http, https, mailto, tel and plain relative links through, and is used at every one of those places; `toastUndo` and the custom form field ids were also fixed. Kept as a regression guard: `supabase/tests/xss-guards.test.mjs` (fails if any escape helper stops escaping quotes, or a raw user link goes into href or src) and `supabase/tests/xss_audit.py` (lists suspicious `${}` in HTML templates for a human to review). Not a proof that no XSS remains: the audit is a heuristic and the remaining list was read by hand. |
...
| F21 | Low | `get-shared-board` used `select("*")` for the normal public share view (not the client portal), so anyone with a share link also received every task column, including attachment links and internal fields. Found during F5. | Fixed and live (23 Sep 2026). `get-shared-board` now selects an explicit column list for the non-portal view too: `id, board_id, title, category, status, due_date, subtasks, position`, the only fields `share.html` renders. Also found and fixed the same leak in `share.html` itself: its fast-path direct query (used whenever a board has no password) was also `select("*")` on both `boards` and `tasks`, bypassing the Edge Function entirely and leaking `share_password_hash`/`share_password_salt` plus every task column straight from the browser. Both trimmed to the same explicit column lists. Deployed via MCP, version 25. Not yet re-tested in a real browser. |
...
node supabase/tests/private-attachments-ui.test.mjs  # no packages needed
```
